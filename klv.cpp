...
/*
* Returns the CRC for 'buffer' of size 'nbytes' bytes.
* To calculate a CRC with multiple calls, pass the returned CRC as the
* 'crc' argument to the next call; otherwise, set ‘crc’ equal to 0.
*/
unsigned short CalcCRC16(const void *buffer, int nbytes, unsigned short crc)
{
    int i;
    unsigned char *buf = (unsigned char *)buffer;
    for (i = 0; i < nbytes; i++)
    {
        crc = ((crc >> 8) ^ CRC16Table[(crc ^ (unsigned short)*buf++) & 0xFF]);
    }
    return crc;
}

/* Ethernet CRC-32 Table */
const unsigned long CRC32Ethernet::m_crcTable[256] = {
...
CRC32Ethernet::CRC32Ethernet(void):m_crcAccum(CRC_ACCUM_INIT)
{
}

unsigned long CRC32Ethernet::Update(char *data_blk_ptr, int data_blk_size)
{
    for (int i = 0; i < data_blk_size; i++)
    {
        m_crcAccum = m_crcTable[(m_crcAccum ^ *data_blk_ptr++) & 0xFF] ^ (m_crcAccum >> 8);
    }
    return m_crcAccum ^ 0xFFFFFFFF;
}
...
int main(int argc, char **argv)
{
    ...
    printf("internal    %x\n", crc.Update((char *)buff, sizeof(buff)));
